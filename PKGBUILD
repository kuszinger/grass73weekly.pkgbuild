...
conflicts=('grass73')
source=('https://grass.osgeo.org/grass73/source/snapshot/grass-7.3.svn_src_snapshot_latest.tar.gz')
md5sums=('SKIP')
noextract=()
...
prepare() {
  ...
  sed -i -e "s,^Name=GRASS GIS 7\$,Name=GRASS GIS ${pkgver}," \
         -e "s,^Exec=grass73\$,Exec=${pkgname}," \
         -e "s,^Icon=grass73\$,Icon=/usr/share/icons/${pkgname}-64x64.png," \
...
package() {
  ...
  sed -i "s,${pkgdir},,g" "${pkgdir}/opt/${pkgname}/include/Make/Platform.make" \
                          ...
                          "${pkgdir}/opt/${pkgname}/bin/grass73" \
                          "${pkgdir}/opt/${pkgname}/demolocation/.grassrc73"

  # Link GRASS exec script in /usr/bin under a custom name.
  # This allows e.g. grass7 and grass7-svn be easily co-installed.
  mkdir -p "${pkgdir}/usr/bin"
  ln -sf "/opt/${pkgname}/bin/grass73" "${pkgdir}/usr/bin/${pkgname}"
  ...
  sed -i '1 s/python/python2/' "${pkgdir}/opt/${pkgname}/bin/grass73"
